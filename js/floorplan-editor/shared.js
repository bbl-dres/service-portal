...
export function editorHeaderHTML(C, session, editMode = false, planCheckHref = '') {
  ...
  return `<header class="fpe-header">
    ...
    ${planCheckHref ? `<a class="btn btn--outline btn--sm btn--icon-left" id="fpe-plan-check" href="${C.escape(planCheckHref)}" target="_blank" rel="noopener noreferrer">${C.icon('CloudUpload', 'btn__icon')}<span class="btn__text">Planprüfung öffnen</span></a>` : ''}
