...
| `test-floorplan-editor.mjs` | Standalone Plan-Editor: exact Workspace deep link, three-pane shell, canonical rooms and deterministic illustrative furniture, camera-aware scale, 3D/walk reference states, selection/URL state, product and structural room editing, module assignment, undo/redo, local save/publish/history/reload, core-data isolation, 320px panel behavior, and portal-shell restoration. |
| `test-floorplan-editor-model.mjs` | Pure Plan-Editor model/repository: all canonical floor baselines, deterministic placements, catalogue-independent baseline token, detached edits, strict local-working-copy round trips and rejection, immutable simulated publications, scoped removal, and bounded undo/redo. |
