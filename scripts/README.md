...
| `test-workspace-knowledge.mjs` | Full workspace-knowledge browser contract: eleven same-origin module images decode and share their first authored image with the eager detail hero; each planning example combines one media-registry context/card photo with three ordered, example-owned generated visualisations; the resulting four-item gallery keeps `Standortfoto` first, then authored visualisation alt text, provenance, and status; scoped queries, legacy slugs, and scoped `WSE:MED` aliases canonicalise; the context item retains media attribution, detail navigation, and licence-gated download state while generated items expose neither a download nor a media-detail action; click/Enter, arrows, Escape, focus restoration, responsive containment, malformed routes, and image-failure behavior remain covered. |
...
| `test-search-answer.mjs` | Pure contracts for the three steps in front of and above the result list: question detection as the cost gate (a dotted identifier is not a question), keyword resolution turning a literal 0 into results, the relevance gate withholding an answer built from a single-word fallback, records without prose never becoming a citation, and the source selection — stored as what is OFF, an empty selection meaning no restriction, and neither jump touching the answer preference. |
