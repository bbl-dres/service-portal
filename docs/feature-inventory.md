...
- `#/app/floorplan-editor?building=<bbl_id>` — dieselbe Portfolioansicht mit vorgewähltem Objekt: Die Karte zoomt darauf und öffnet sein Popup mit Adresse, Planstand, offenen Hinweisen, Geschossliste und beiden Handoffs.
...
- Die Arbeitsfläche trägt einen echten Breadcrumb «Portfolio › Gebäude › Geschoss». Rücksprung, Gebäude-Breadcrumb und Marke führen deterministisch zur Portfolioansicht, der Gebäude-Breadcrumb mit vorgewähltem Objekt; keiner dieser Links hängt von der Browserhistorie oder der zuvor besuchten Portalansicht ab. Die bestehenden `floorplan.js`-Renderer von Workspace und Mietendenportal bleiben reine Lesekonsumenten.
...
- `Planprüfung öffnen` ist ein echter Handoff in die getrennte Planprüfung: Die Arbeitsliste übergibt Gebäude und Geschoss der betroffenen Aufgabe, die Portfolioansicht das gewählte Gebäude und die Arbeitsfläche Gebäude plus Geschoss. Die Zielroute nimmt lokale DWG-Testdateien entgegen; der Editor selbst importiert weiterhin keine DWG, führt keine Regeln aus und speichert kein Prüfresultat.
