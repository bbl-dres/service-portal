...
- Eintragsdetails verwenden echte Textregister nach APG-Muster: «Übersicht» plus «Attribute», «Felder» oder «Werte». Die Übersicht zeigt Beschreibung, Kerndaten sowie gegebenenfalls technische Angaben und Quellen links; rechts bündelt eine kompakte Karte «Ansprechpersonen» Kontakt- und Verantwortungsangaben. Datentabellen mit sicherer Quell-URL erhalten davor eine Aktionskarte «Systemzugang».
...
- Eintragsübersichten nutzen innerhalb von `pf-layout--detail` ein `detail-layout`: links folgen Beschreibung und direkte `kv`-Raster in vertikalen Abschnitten, rechts steht nur in «Übersicht» die gemeinsame Karte «Ansprechpersonen» und bei Datentabellen optional davor «Systemzugang». Auf kleinen Viewports und während des 1024–1279-px-Hierarchiewechsels folgen die Karten unter dem Inhalt; ab 1280 px bilden sie die 22-rem-Randspalte.
...
- Aktionen exportieren CSV/Excel oder öffnen den Druckdialog. Interne Links verbinden Attribute und Felder sowie Datentabellen und veröffentlichte Datensätze; die optionale Karte «Systemzugang», weitere externe Quellen und AdminDir-Einträge verwenden abgesicherte Links.
...
- Beide Prozessarten verwenden in «Übersicht» dasselbe `detail-layout`: Beschreibung, Einordnung sowie — sofern vorhanden — Schlagwörter stehen links; rechts bündelt eine Karte «Ansprechpersonen» Fachstelle und Verantwortungsrollen. Diagramm und Prozessschritte bleiben vollbreit.
...
- `detail-layout` und `pf-layout--detail` ordnen auf schmalen Viewports Prozessinhalt, die gemeinsame Übersichtskarte und sekundäre Hierarchie in dieser Reihenfolge; zwischen 1024 und 1279 px bleibt die Karte unter dem Inhalt, ab 1280 px bildet sie eine 22-rem-Randspalte. Detailregister scrollen intern und erzeugen keinen Dokument-Overflow.
