...
| `#/app/floorplan-editor?mode=<mode>&sort=<key>&obj=<bbl_id>&view=work&layer=<layer>` | Startseite des Plan-Editors: kartenzentrierte Portfoliosuche (Standard) und Arbeitsliste (Login-Gate) |
| `#/app/floorplan-editor?building=<bbl_id>&tab=<register>&plans=<mode>` | Objektdetail des Plan-Editors mit Übersicht, Grundrissen und Ausstattungen (Login-Gate) |
...
- `#/app/floorplan-editor?building=<bbl_id>&tab=uebersicht|grundrisse|ausstattungen&plans=cards|list` — Objektdetail in der Anatomie des Liegenschaften-Inventars, aber aus Sicht des Flächenmanagements: Breadcrumb zurück ins Portfolio, Titel, Planstand-Badge, Kennzahlenband (Geschosse, Räume, HNF, Arbeitsplätze) und drei CD-Register. «Übersicht» führt den Geschoss-Schnellzugriff mit Miniaturvorschau, die Eckdaten und die Planübernahme sowie Aktions- und Kontaktkarte des Portals; «Grundrisse» die Geschosse als Galerie mit Planvorschau oder als reine Faktentabelle mit eigener Katalogleiste — Vorschaubilder erscheinen nur auf Karten, nie in einer Tabellenzeile; «Ausstattungen» den Multispace-Ausstattungsstandard je Modulgruppe mit Anteil und die Ausstattung je Geschoss. Ohne `tab` gilt «Übersicht», ohne `plans` die Galerie. Objekte ohne CAD-Planübernahme erklären ihren leeren Ausstattungsbestand.
