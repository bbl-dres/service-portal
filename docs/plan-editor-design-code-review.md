...
5. Der Viewer-Interaktionsreview ersetzt die richtungsabhängige 3D-Pan-Näherung durch ein Greifen der Bodenebene unter dem Zeiger. Der mathematische Fallback skaliert wie Three.js `OrbitControls` mit Distanz, FOV und Viewporthöhe. Rotation ist viewportbezogen; Ziel, Pitch, Distanz und Kamerakollision sind begrenzt. Einpassen berücksichtigt FOV und Seitenverhältnis, Zoom bleibt am Zeiger verankert und der Fit-Faktor wird über Responsive-Resizes erhalten. Ein Finger verschiebt; zwei Finger verschieben, drehen und zoomen. Sichtbare Wände blockieren die Auswahl verdeckter Objekte.
6. Die 2D-Kamera schreibt bei Pan und Zoom nur noch die SVG-`viewBox` statt den kompletten Plan neu aufzubauen. Wheel-Deltas werden für Pixel-, Zeilen- und Seitenmodus kontinuierlich normalisiert; ein `ResizeObserver` erhält Plan-Einheiten pro CSS-Pixel über Panel- und Viewportwechsel. Gesten verwenden eine Transformmatrix vom Pointer-Start, Autorenbewegungen CSS-Pixel-Schwellen und exakten Rollback bei Jitter/Abbruch. Touch-Pinch kann sauber in Ein-Finger-Pan übergehen; die mittlere Maustaste verschiebt auch während eines aktiven Autorenwerkzeugs. Skalierte unsichtbare Trefferflächen halten Raumgriffe bei jeder Zoomstufe bedienbar, ohne sie visuell zu vergrössern.

Die Kameraregeln orientieren sich an der offiziellen Three.js-Implementierung von [`OrbitControls`](https://raw.githubusercontent.com/mrdoob/three.js/r184/examples/jsm/controls/OrbitControls.js), der Anleitung für [responsive Renderer](https://threejs.org/manual/en/responsive.html) und dem dokumentierten [Shadow-Map-Lebenszyklus](https://threejs.org/docs/pages/WebGLRenderer.html#ShadowMap). Das gewünschte Mapping «links verschieben, rechts drehen» entspricht dabei dem Interaktionsmodell von [`MapControls`](https://threejs.org/docs/pages/MapControls.html).

Vor dem Produktionsausbau bleiben bewusst Systemgrenzen offen, die dieser Browser-Prototyp nicht vortäuschen darf:

1. Rendering und Suche müssen mit realistischen Grossplänen profiliert werden. Bei mehreren hundert gleichzeitig sichtbaren Räumen und tausenden Objekten können weiterhin inkrementelle SVG-Updates oder Listen-Virtualisierung nötig werden.
2. Die lokale Publikation ist keine Sicherheits- oder Kollaborationsgrenze. Produktion benötigt eigenständige Raum-Entitäten, authentifizierte API-Aufrufe, Row-Level Security, serverseitige Autorenschaft, Planrevisionen und echte optimistische Konfliktprüfung.
3. Das erzeugte 3D-Modell ist semantisch an das aktuelle Editor-Dokument gebunden, aber geometrisch weiterhin eine Näherung. CAD-Wand-/Türtopologie, Kollisionsprüfung, importierte Produktmodelle und Planprüfung gehören in spätere, getrennt spezifizierte Ausbauschritte.
...
- `node scripts/test-floorplan-editor-three-controls.mjs`
- `git diff --check`

Der Browserlauf deckt Login-Gate, Gebäude-/Geschossnavigation, Deep Links, Dreispaltenlayout, Ressourcenhierarchie, editmodusspezifische Werkzeugreihenfolge, kontextuelle Bibliothek, Strukturmenü/-Lock, Aktionsmenüs, Tastaturbedienung, 2D-Pan/Wheel/Pinch und Touch-Handoff, mittlere Maustasten-Navigation im Autorenwerkzeug, 3D-Boden-Pan/Rotation/Zoom-to-Cursor/Pinch, responsive Kamera-Fits, 3D/Begehung, Kameraerhalt, Selektion, Edit-Jitter und Pointer-Abbruch, Undo/Redo, lokale Arbeitskopie/Publikation, kanonische Datenisolation sowie die 320-px-Drawer- und Breakpoint-Zustände ab.
