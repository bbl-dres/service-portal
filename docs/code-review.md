...
- **Merkmalliste `.kv`**: Doppelpunkt an jeder Beschriftung — mit Abstand davor («Baujahr :»), aus dem Blatt (`dt::after`) statt aus neun Zeichenketten; das Leerzeichen ist geschützt (`\00A0`), damit der Doppelpunkt bei einer umbrechenden Beschriftung nicht allein auf der nächsten Zeile landet — und eine spürbar getrennte Beschriftungsspalte (`fit-content(18rem)` + 2rem Spalte­nabstand). Beim Nachmessen fiel auf, dass die zuerst geschriebene Fassung `minmax(0, min(18rem, max-content))` **ungültig** war — `max-content` ist keine Länge, `min()` verwirft die ganze Angabe und `.kv` fiel auf eine Spalte zurück. `fit-content()` ist die richtige Grundform.
